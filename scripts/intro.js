HTomb = (function(HTomb) {
  ...
  HTomb.Intro.setup = function() {
    ...
    necro = HTomb.Things.Necromancer.spawn();
    HTomb.Debug.necro = necro;
    ...
    maze.create(function(x,y,val) {
      if (val===0 && HTomb.World.tiles[z][x+1][y+1]===HTomb.Tiles.WallTile) {
        HTomb.Things.DigTask.spawn({assigner: necro}).place(x+1,y+1,z);
      }
      //if (val===0) {
    ...
    throne = HTomb.Things.Throne.spawn();
    throne.place(Math.floor(width/2),Math.floor(height/2),z);
    HTomb.Things.FakeTorch.spawn().place(throne.x+3, throne.y-1, z);
    HTomb.Things.FakeTorch.spawn().place(throne.x-3, throne.y-1, z);
    HTomb.Things.Pentagram.spawn().place(throne.x, throne.y+2, z);
    for (let i=0; i<nzombies; i++) {
      let zombie = HTomb.Things.Zombie.spawn();
