...
namespace Hecatomb
{
    ...
    /// </summary>
    public class UnclaimTask : Task, ISelectsZone, IMenuListable
    {
        ...
        public override void TileHover(Coord c)
        {
            ...
            co.MenuMiddle.Clear();
            co.MenuMiddle = new List<ColoredText>() { "{green}" + String.Format("Unclaim items from {0} {1} {2}", c.X, c.Y, c.Z) };
        }
        public override void TileHover(Coord c, List<Coord> squares)
        {
            var co = Game.Controls;
            co.MenuMiddle.Clear();
            co.MenuMiddle = new List<ColoredText>() { "{green}" + String.Format("Unclaim items to {0} {1} {2}", c.X, c.Y, c.Z) };
        }
