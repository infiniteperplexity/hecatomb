...
namespace Hecatomb8
{
    ...
    public class TutorialHandler : StateHandler
    {
        ...
        public List<ColoredText> GetText()
        {
            ...
            if (Current.Name == "Log")
            {
                list = list.Concat(InterfaceState.Controls.InfoTop).ToList();
                list.Add(" ");
            ...
            else if ((Current.Name == "ChooseDigTiles" || Current.Name == "ChooseBuildTiles") && InterfaceState.Controls is SelectZoneControls)
            {
                list = Current.ControlText.ToList();
                list = list.Concat(Current.InstructionsText).ToList();
                if (!(InterfaceState.Controls as SelectZoneControls)!.FirstCorner.Equals(default(Coord)))
                {
                    list[2] = "{yellow}Select second corner with keys or mouse.";
                }
                if (Current.ShowTimeAndSanity > -1)
                {
                    var ins = Time.GetTimeText();
                    ins.Add(" ");
                    ins.Add(GameState.World!.Player!.GetComponent<SpellCaster>().GetSanityText());
                    list.InsertRange(Current.ShowTimeAndSanity, ins);
                }
            }
            else if (!Current.RequiresDefaultControls || InterfaceState.Controls is DefaultControls)
            {
                list = Current.ControlText.ToList();
                list = list.Concat(Current.InstructionsText).ToList();
                if (Current.ShowTimeAndSanity > -1)
            ...
            else if (InterfaceState.Controls == InterfaceState.CameraControls)
            {
                list = list.Concat(InterfaceState.Controls.InfoTop).ToList();
                list.Add(" ");
        ...
        public TutorialHandler()
        {
            ...
            SlopeCount = 0;
            CurrentIndex = 0;
            Visible = !HecatombOptions.NoTutorial;
        ...
        public GameEvent HandleEvent(GameEvent g)
        {
            ...
            if (g is TutorialEvent)
            {
                ...
                TutorialStates[CurrentIndex].HandleEvent(t);
            }   
            return g;
        }
